...
export default function Complicated_Slider({stock,tour_id}:{stock:Stock,tour_id:number}) {
  let {tours,fetchTours} = useTourStore()
  useEffect(()=>{
    fetchTours()
  },[fetchTours])
  let tour = (tours as Tour[]).find((el) => el.id === tour_id)
  const [year, month, day] = tour?.date.start_date.split("-") ?? ["", "", ""];
  const formatted = tour ? `${month}/${day}/${year.slice(2)}` : "";
        ...
        <div className="body w-full h-full flex items-center justify-between gap-20">
            ...
            <div className="stockCard mt-10">
               <div className=" text-gray-50 px-[0px] py-[50px] flex flex-col gap-1 mb-13">  
                  ...
                  <Button title={`buy`} type='third' Effectclass='max-w-[80px] mt-4' onClick={()=>{open("buy",tour_id)}}/>
                </div>
            </div>
        </div>
