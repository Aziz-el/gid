...
export default function My_Tours({ activePage }: { activePage: number }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const resData = response.data.filter((item:{id:number,agency_id:number}) =>
        companyData?.id == item.agency_id
      )
      setData(resData)
      console.log(resData);
  ...
  return (
    <div className="favs w-[900px] relative">
      ...
      >
        ...
        )):( <SwiperSlide><div className='w-[900px] h-[500px] flex items-center '><h1 className='flex justify-center'>Пока что нету твоих туров</h1></div></SwiperSlide>)}
      </Swiper>
    </div>
  )
}
