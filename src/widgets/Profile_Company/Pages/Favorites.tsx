...
export default function Favorites_Company({ activePage }: { activePage: number }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const resData = response.data.filter((item: { id: number }) =>
        companyData?.favorites?.includes(item.id)
      )
      setData(resData)
    }
    fetchData()
  ...
  return (
    <div className="favs w-[900px] relative">
      ...
      >
        ...
        )):( <SwiperSlide><div className='w-[900px] h-[500px] flex items-center '><h1 className='flex justify-center'>Пока что нету туров</h1></div></SwiperSlide>)}
      </Swiper>
    </div>
  )
}
